...
const Testimonials = () => {
  return (
    ...
    >
      {/* Decorative Elements */}
      <div className="absolute top-20 right-10 w-64 bg-violet-200/30 rounded-full blur-3xl"></div>
      <div className="absolute bottom-20 left-10 w-96 bg-violet-200/30 rounded-full blur-3xl"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {TESTIMONIALS.map((testimonial, index) => (
            <div
              key={index}
              className="group relative bg-white/80 backdrop-blur-sm rounded-3xl p-8 border border-gray-100 hover:border-violet-200 hover:shadow-2xl hover:shadow-violet-50/10 transition-all duration-300 hover:-translate-y-2"
            >
              {/* Quote Icon */}
              ...
              {/* Hover Gradient Backgroud */}
              <div className="absolute inset-0 bg-gradient-to-br from-violet-50/0 to to-purple-50/0 group-hover:from-violet-50/50 group-hover:to-purple-50/30 rounded-3xl transition-all duration-300 -z-10"></div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-20 grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-4xl mx-auto">
          ...
          <div className="text-center">
            ...
            <div className="text-gray-600">Ebook Created</div>
          </div>
        </div>
      </div>
    </div>
